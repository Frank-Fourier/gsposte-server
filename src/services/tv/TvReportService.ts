...
import { TvReport, tvReportDecoder, TvReportDocument, TvReportModel } from "@models/tv/TvReportModel";
...
const uploader = multer({
    storage: diskStorage({
        ...
        filename(req: Request, file: Express.Multer.File, callback: (error: (Error | null), filename: string) => void): void {
            callback(null, `attachment_${Date.now()}${path.extname(file.originalname)}`);
        }
    }),
    limits: {
        files: 1,
...
export class TvReportService extends MongoRepository<TvReport, TvReportDocument> {
     ...
     * @returns Promise resolved with file name when the upload is done.
     */
